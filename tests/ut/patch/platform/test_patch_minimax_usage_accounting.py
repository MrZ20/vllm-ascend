...
def test_full_generator_forwards_chat_template_kwargs_on_target_main(monkeypatch):
    monkeypatch.setattr(minimax_usage_patch, "vllm_version_is", lambda _: False)
    calls: dict[str, Any] = {}
    parser_calls = []
    ...
    async def original_full_generator(*args, **kwargs):
        calls["args"] = args
        calls["kwargs"] = kwargs
    ...
    asyncio.run(
        minimax_usage_patch._wrapped_chat_completion_full_generator(
            fake_serving,
