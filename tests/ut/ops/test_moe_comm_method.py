...
from vllm_ascend.ops.fused_moe.token_dispatcher import TokenCombineResult, TokenDispatchResult
...
class TestMoECommMethod(TestBase):
    ...
    def test_all_gather_comm_impl(self, mock_token_dispatcher, mock_prepare_finalize, mock_get_forward_context):
        ...
        mock_pf_instance.prepare.return_value = (torch.randn(4, 8), torch.randn(4, 2), None, None)
    ...
    def test_mc2_comm_impl(self, mock_token_dispatcher, mock_prepare_finalize, mock_get_forward_context):
        ...
        # Mock prepare finalize
        mock_pf_instance = MagicMock()
        mock_pf_instance.prepare.return_value = (torch.randn(4, 8), torch.randn(4, 2), torch.tensor([1, 0, 1, 0]), None)
        mock_pf_instance.finalize.return_value = torch.randn(4, 8)
        mock_prepare_finalize.return_value = mock_pf_instance
    ...
    def test_fused_experts_method(
        ...
        # Mock prepare finalize
        mock_pf_instance = MagicMock()
        mock_pf_instance.prepare.return_value = (torch.randn(4, 8), torch.randn(4, 2), None)
        mock_pf_instance.finalize.return_value = torch.randn(4, 8)
        mock_prepare_finalize.return_value = mock_pf_instance
        ...
        mock_td_instance.token_dispatch.return_value = TokenDispatchResult(
            hidden_states=torch.randn(6, 8), group_list=torch.tensor([2, 2, 2]), group_list_type=1
        )
