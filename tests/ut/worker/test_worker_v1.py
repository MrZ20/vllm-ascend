...
class TestNPUWorker(TestBase):
    ...
    def test_profile_restart_reuses_existing_profiler(self):
        """[RFC #6954] Restart profile (stop then start) reuses existing profiler, don't call _create_profiler again"""
        from vllm_ascend.worker.worker import NPUWorker
    ...
    def test_execute_model_kv_connector_not_finished(
        ...
        with patch.object(NPUWorker, "__init__", lambda x, **kwargs: None):
            ...
            # When both finished_sending and finished_recving are False, return EMPTY_MODEL_RUNNER_OUTPUT directly
