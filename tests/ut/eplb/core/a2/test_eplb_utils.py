...
class TestAscendConfig(unittest.TestCase):
    ...
    def setUp(self, mock_fix_incompatible_config):
        ...
        if vllm_version_is("0.22.1"):
            ...
            # vLLM PR #41184 (FusedMoE/MoERunner inversion refactor) made
            # `intermediate_size` a required field of FusedMoEConfig and derives
            # `intermediate_size_per_partition = intermediate_size // tp_size`
            # in __post_init__. It also changed `activation` from a string to
            # MoEActivation, whose `is_gated` property is read during
            # __post_init__. tp_size=2 here, so intermediate_size=10 yields the
            # same intermediate_size_per_partition=5 as the legacy v0.22.1
            # construction above.
            moe_config = FusedMoEConfig(
                num_experts=8,
                experts_per_token=8,
