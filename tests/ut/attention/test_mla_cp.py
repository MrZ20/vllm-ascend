...
class TestAscendMLAImpl(TestBase):
    ...
    def test_compute_prefill_context_with_dcp_pcp(self, mock_all2all, mock_dcp, mock_pcp, mock_ring, mock_load):
        def mock_ring_attn(
            q_nope,
            q_rope,
            k_nope,
        ...
        for test_case in test_cases:
            ...
            prefix_lse = torch.randn(sum(nums_tokens_per_rank), self.impl.num_heads, dtype=torch.float16)
    ...
    @patch("torch_npu.atb.npu_ring_mla")
    def test_attention_with_mask_and_nomask_with_dcp_pcp(self, mock_npu_ring_mla):
        ...
        def mock_npu_ring_mla_effect(
            ...
            return torch.randn(q_nope.shape[0], value.shape[1], value.shape[-1])

        mock_npu_ring_mla.side_effect = mock_npu_ring_mla_effect
        test_cases = [([8], 2, 2), ([8], 2, 1), ([8], 1, 2), ([8], 2, 2), ([8, 12], 2, 2)]
        for test_case in test_cases:
            ...
            for rank in range(pcp_size):
                (
                    ...
                    kv_with_q_head_nomask_seqlens,
                    kv_with_q_tail_nomask_seqlens,
                ) = get_pcp_split_info(rank, pcp_size, nums_tokens_per_rank)
                ...
                self.assertEqual(lse_head.shape, (num_heads, q_head_idx.shape[0]))
                self.assertEqual(mock_npu_ring_mla.call_count, 1 + (len(kv_with_q_head_nomask_idx[0]) != 0))
                ...
                self.assertEqual(output_tail.shape, (q_tail_idx.shape[0], num_heads, v_head_dim))
                self.assertEqual(lse_tail.shape, (num_heads, q_tail_idx.shape[0]))
                self.assertEqual(mock_npu_ring_mla.call_count, 1 + (len(kv_with_q_tail_nomask_idx[0]) != 0))
                mock_npu_ring_mla.reset_mock()

    @patch_distributed_groups(dcp_size=2, pcp_size=2)
    def test_process_attn_out_lse_with_dcp_pcp(self, mock_all_to_all, mock_dcp, mock_pcp):
    ...
    def test_forward_prefill_cp_with_dcp_pcp(self, mock_npu_ring_mla):
        def mock_attention_with_nomask_and_mask(
            ...
            mock_output = torch.randn(
                q_nope.shape[0], self.impl.num_heads, self.impl.v_head_dim, dtype=k_pe.dtype, device=k_pe.device
            )
            mock_lse = torch.randn(self.impl.num_heads, q_pe.shape[0], dtype=torch.float32, device=k_pe.device)
            return mock_output, mock_lse

        def mock_compute_prefill_context(
        ...
        block_size = 32
        kv_c_and_k_pe_cache = (
            torch.randn(block_num, block_size, 1, self.impl.q_lora_rank, dtype=torch.float16),
            torch.randn(block_num, block_size, 1, self.impl.qk_rope_head_dim, dtype=torch.float16),
        )
        test_cases = [([8], 2, 2), ([8], 2, 1), ([8], 1, 2), ([8], 2, 2), ([8, 16], 2, 2)]
        for test_case in test_cases:
            scheduled_tokens, pcp_size, dcp_size = test_case
            nums_tokens_per_rank = []
            for num_tokens in scheduled_tokens:
                assert num_tokens % (2 * pcp_size) == 0  # padded head&tail compute balance
                nums_tokens_per_rank.append(num_tokens // pcp_size)
            seq_len_q, seq_len_k = sum(nums_tokens_per_rank), sum(scheduled_tokens)

            q_nope = torch.randn(seq_len_q, self.impl.num_heads, self.impl.qk_nope_head_dim, dtype=torch.float16)
            q_pe = torch.randn(seq_len_q, self.impl.num_heads, self.impl.qk_rope_head_dim, dtype=torch.float16)
            k_nope = torch.randn(seq_len_k, self.impl.num_heads, self.impl.qk_nope_head_dim, dtype=torch.float16)
            k_pe = torch.randn(seq_len_k, self.impl.num_heads, self.impl.qk_rope_head_dim, dtype=torch.float16)
            value = torch.randn(seq_len_k, self.impl.num_heads, self.impl.v_head_dim, dtype=torch.float16)
            # only test one rank
            for rank in range(pcp_size):
                (
                    q_head_idx,
                ...
                attn_metadata.prefill.pcp_metadata.q_full_idx = torch.cat(
                    [attn_metadata.prefill.pcp_metadata.q_head_idx, attn_metadata.prefill.pcp_metadata.q_tail_idx]
                )
                ...
                self.assertEqual(output.shape, (seq_len_q, self.impl.num_heads * self.impl.v_head_dim))
