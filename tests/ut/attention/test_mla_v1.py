...
class TestAscendMLAImpl(TestBase):
    ...
    def test_forward_decode_without_graph(
        self, mock_npu_fused_infer_attention_score, mock_up_proj, mock_get_forward_context
    ):
        num_tokens = 100
        block_size = 4
    ...
    @patch("vllm_ascend.ascend_forward_context.get_forward_context")
    @patch("torch_npu.npu_fused_infer_attention_score")
    def test_forward_decode(self, mock_npu_fused_infer_attention_score, mock_get_forward_context):
        B = 2
        N = self.impl.num_kv_heads
        BS = 100
        ...
        mock_npu_fused_infer_attention_score.return_value = [torch.randn(B, N, self.impl.kv_lora_rank), None]
        mock_get_forward_context.return_value = MagicMock(capturing=False)
