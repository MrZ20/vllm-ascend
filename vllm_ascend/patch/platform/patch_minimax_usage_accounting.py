...
def _resolve_reasoning_parser_for_usage(
    ...
    if vllm_version_is("0.22.1"):
        # vLLM PR #45190 has not landed in v0.22.1, so the old OpenAI serving
        # generator still receives reasoning_parser directly from
        # _create_chat_completion.
        return reasoning_parser
    else:
        # vLLM PR #45190/#45171 refactored OpenAI chat serving so generators
        # receive chat_template_kwargs instead of a reasoning_parser object.
        # Recreate the parser for usage accounting; upstream still builds the
        # same parser in _create_chat_completion for engine-side kwargs.
...
async def _wrapped_chat_completion_stream_generator(
    ...
    **extra_kwargs: Any,
    ...
    if vllm_version_is("0.22.1"):
        # v0.22.1 OpenAI serving still accepts reasoning_parser as a generator
        # argument. Keep that call shape unchanged for the legacy branch.
        ...
        # vLLM PR #45190/#45171 removed reasoning_parser from generator
        # signatures and threads parser defaults via chat_template_kwargs.
        stream_generator = original_stream_generator(
            request,
            _tracked_result_generator(result_generator, state),
            request_id,
...
async def _wrapped_chat_completion_full_generator(
    ...
    **extra_kwargs: Any,
    ...
    if vllm_version_is("0.22.1"):
        ...
        # vLLM PR #45190/#45171 removed reasoning_parser from the full
        # generator signature and added chat_template_kwargs. Forward target
        # main kwargs explicitly so parser creation sees the request defaults.
        response = await original_full_generator(
            request,
            _tracked_result_generator(result_generator, state),
            request_id,
