...
# Target-main-only patch set. The v2 worker patches depend on target-main-only
# APIs that diverged from the v0.22.1 release, so v0.22.1 keeps the legacy v1
# runner path unchanged rather than carrying a separate compatibility shim. With
# v0.22.1 installed this flag is False, so none of the patch_v2.* /
# routed-experts-capture patches below are imported and the v2 worker stays
# dormant (the release uses the v1 runner).
...
if not vllm_version_is("0.22.1"):
    ...
    # generic/FP8 weight loading needs the target-main-only name remap patch.
