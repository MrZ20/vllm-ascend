...
class AscendModelSlimConfig310(AscendModelSlimConfig):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
        ...
        elif isinstance(layer, AscendRoutedExperts310):
            if self.is_layer_skipped_ascend(prefix, self.packed_modules_mapping):
                logger.debug(
                    "Select AscendUnquantizedFusedMoEMethod310 for %s (layer=%s)",
                    prefix,
                    "AscendRoutedExperts310",
                )
            ...
            logger.debug("Select AscendFusedMoEMethod for %s (layer=%s)", prefix, "AscendRoutedExperts310")
