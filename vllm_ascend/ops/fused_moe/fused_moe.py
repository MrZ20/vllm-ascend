...
if TYPE_CHECKING:
    ...
    # subclassable owner of MoE weights and helper methods, and the unquantized
    # method still lives in layer.py. Do not import the target-main-only
    # RoutedExperts owner here: that semantic surface must not be visible to the
    # v0.22.1 runtime or tests.
    from vllm.model_executor.layers.fused_moe.layer import UnquantizedFusedMoEMethod
else:
    # vLLM PR #41184 changed FusedMoE into a factory and moved the subclassable
    # weight owner to RoutedExperts. Use explicit version branches instead of
    # import probing so main/v0.22.1 compatibility is deterministic.
    from vllm.model_executor.layers.fused_moe.layer import (
        fused_moe_make_expert_params_mapping as _target_fused_moe_make_expert_params_mapping,
    )
    from vllm.model_executor.layers.fused_moe.routed_experts import RoutedExperts as _TargetRoutedExperts
...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def apply(
        ...
        if topk_weights is None or topk_ids is None:
            ...
            # those tensors on target main; v0.22.1 never sets them (its legacy
            # forward path always selects experts here), so this branch is inert
            # on v0.22.1 and the runtime behaviour stays identical to main.
...
# two supported versions as two complete implementations wrapped in one big
# if/else: the v0.22.1 branch is a verbatim copy of the pre-#41184 main source
# (FusedMoE is still the subclassable layer that owns MoE weights, and MoERunner
# delegates compute back to the layer), while the else branch is the target-main
# (#41184) factory/RoutedExperts implementation. Keeping the whole legacy path in
# a single block means it can be deleted wholesale once v0.22.1 support is
# dropped, instead of hunting down scattered version checks.
if vllm_version_is("0.22.1"):
    ...
    def patch_fused_moe_factory(replacement=None) -> None:
        ...
        # class and CustomOp OOT registration handles replacement. There is no
        # factory to patch on the legacy branch, so this is a no-op.
        return
    ...
    class AscendFusedMoE(torch.nn.Module):
        # Upstream vLLM PR #41184 turned FusedMoE into a factory returning
        # MoERunner, so AscendFusedMoE is a factory shell on target main: __new__
        ...
        moe_counter = -1
        gate_stream: torch.npu.Stream | None = None
        if _target_fused_moe_make_expert_params_mapping is not None:
    ...
    class AscendRoutedExperts(_TargetRoutedExperts):
        ...
        def __init__(
            ...
            **kwargs,
            ...
            # class to RoutedExperts, but Ascend still reuses the same NPU MoE
            # forward path. Keep the routing fields that the Ascend forward path
            # reads as public attributes even if upstream RoutedExperts stores or
            # normalizes them differently.
