...
这份大纲是第二版教学路线。它不再只讲“调用了哪些函数”，而是沿着一次真实推理的数据流，把每个关键阶段的技术原理、公式和源码入口串起来。
...
第一轮先写了 `000-004`，用于建立基础概念。第二轮先从最核心的数据推导和模型数学写起，所以编号从 `010` 开始，给中间运行时章节留出了空间。现在 `005-009` 已补齐，完整阅读路径是：
